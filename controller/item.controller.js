...
export const production_AddQuantity = async (req, res) => {
  try {
    ...
    console.log("admin ente", payload);
    let isUser = await UserSchema.findOne({ _id: userId });
    if (!isUser || isUser.role === "buyer") {
      return res.status(422).send({ message: "You Are Not Authorized" });
    }
    ...
    try {
    //   let orderCheck = await ItemModel.find({ _id: id });
    //   console.log("produc", orderCheck);
    //   let newQuant = orderCheck.product_quantity + +payload.quantity;
    //   orderCheck.product_quantity = newQuant;
    //   console.log("ptt", orderCheck.product_quantity);
      //  let updated =  orderCheck.save();
